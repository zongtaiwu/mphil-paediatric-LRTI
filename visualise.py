...
def makeFunctionalNetworkPlot(G, layoutstyle, fc_dict):
    # kamada kawai layout uses as few crossing edges as possible
    # multipartite layout puts membrane proteins left, cytosol proteins middle and nucleus proteins right
    ...
    for node in nodes:
        if node in membraneprot:
            ...
            layers[node] = 1
    nx.set_node_attributes(G, layers, 'layers')
    #pd.DataFrame(membrane_G).to_csv('../data/subsubnetwork_membrane_genes.txt', header = None, index = False)
    # generate some colormaps
    # give input proteins black node edge
    color_map_edges = []
    for node in nodes:
    ...
    pos_multipartite = nx.multipartite_layout(G,subset_key = 'layers') 
    for node in nodes:
        pos_multipartite[node][0] = pos_multipartite[node][0] + (0.01*(np.random.random_sample(1) - 0.02))
    
    pos_kamadakawai = nx.layout.kamada_kawai_layout(G, weight=None, pos=pos_multipartite) 
    ...
    dict_colors = {0.1: 'r', 1: 'sandybrown', 2: 'limegreen'}
    listcol = []
    for element in list(labels.values()):
        listcol += [dict_colors[element]]
    nx.draw(G,pos=pos,node_color=fc_cmap, edge_color = listcol,  with_labels=True, font_size = 20,node_size = eigen_centrality, edgecolors = color_map_edges)             
    
    return fig

def transformIntoFunctionalNetwork(G, sif_address, gridplot_address):
